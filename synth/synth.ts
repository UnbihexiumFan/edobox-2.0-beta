    ...
    public fromJsonObject(patternObject: any, song: Song, channel: Channel, importedPartsPerBeat: number, isNoiseChannel: boolean, isModChannel: boolean): void {
        ...
        if (patternObject["notes"] && patternObject["notes"].length > 0) {
            ...
            for (let j: number = 0; j < patternObject["notes"].length; j++) {
                ...
                const maxPitch: number = isNoiseChannel ? Config.drumCount - 1 : song.maxPitch;
...
export class Instrument {
    ...
    public static frequencyFromPitch(pitch: number, edo: number): number {
        return 440.0 * Math.pow(2.0, (pitch - 69.0) / edo);
    }
...
export class Song {
    ...
    public initToDefault(andResetChannels: boolean = true): void {
        ...
        this.key = 0;
        this.edo = 12;
        this.maxPitch = 12 * Config.pitchOctaves;
        this.loopStart = 0;
        this.loopLength = 4;
        this.tempo = 150;
        this.reverb = 0;
    ...
    public toBase64String(): string {
        ...
        //buffer.push(SongTagCode.maxPitch, base64IntToCharCode[this.maxPitch]);
    ...
    public fromBase64String(compressed: string): void {
        ...
        while (charIndex < compressed.length) switch (command = compressed.charCodeAt(charIndex++)) {
            ...
            //case SongTagCode.maxPitch: {
            //    this.maxPitch = base64CharCodeToInt[compressed.charCodeAt(charIndex++)];
            //} break;
    ...
    public toJsonObject(enableIntro: boolean = true, loopCount: number = 1, enableOutro: boolean = true): Object {
        ...
        return {
            ...
            "edo": this.edo,
            //"maxPitch": this.edo * Config.pitchOctaves,
...
class InstrumentState {
    ...
    public updateWaves(instrument: Instrument, samplesPerSecond: number, edo: number): void {
        ...
        if (instrument.type == InstrumentType.chip) {
            ...
            for (let i: number = 0; i < Config.drumCount; i++) {
                this.drumsetSpectrumWaves[i].getCustomWave(instrument.drumsetSpectrumWaves[i], InstrumentState._drumsetIndexToSpectrumOctave(i, edo));
            }
            this.wave = null;
        } else {
            this.wave = null;
